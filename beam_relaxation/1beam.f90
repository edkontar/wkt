...
! -----------------  data output to the screen ------------
   ...
   write(*,'(A,E7.1,A)')'Pl Freq. Omega_pe=', Omega,' s^{-1}'
   write(*,'(A,E7.1,A)')'Collisional Freq     =', coll_ei,' s^{-1}'
...
!          w2=-a2*W(i)*(f(i)-f(i-1))*v_t/(dk)
   	   
	 ! spectral energy density calsculated at step t+dt
	   
!	    !m1= (w(i+1)*kx(i+1)**3+w(i)*kx(i)**3)*(f(i+1)-f(i))/2.
!            m1= (w(i+1)*kx(i+1)**3)*(f(i+1)-f(i))
!        if (i==2) then
!	    m2= 0.
!	    else
!	    !m2= (w(i-1)*kx(i-1)**3+w(i)*kx(i)**3)*(f(i)-f(i-1))/2.
!             m2= (w(i)*kx(i)**3)*(f(i)-f(i-1))
!	    end if

 ! now we advance functions to one step 

!        w(i) = w(i)&! - grad*(w(j)-w(j-1))*dt/dk&
!                -a2*W(i)*(f(i)-f(i-1))*v_t/(dk)*dt
!        f(i) =f(i) + a1*dt*(m1-m2)*kx(i)*kx(i)/(dk*dk*v_t**3)
          	
!	  END DO
! the subroutine calculates the spectrum
